...
class Visualizer:
    ...
    def plot_system(self, system: AttractorSystem):
        ...
        if system.dimensions >= 3:
            ...
            ax3d.set_ylabel('Y')
            ax3d.set_zlabel('Z')
            ax3d.set_title(f'3D Attractor\nLyapunov: {system.lyapunov:.3f}')

            plt.colorbar(scatter, label='Time Evolution', orientation='horizontal')
            ...
            print("Plotting is only implemented for 3D systems.")

    def animate_system(self, system: AttractorSystem, filename: str = "attractor_animation.mp4", fps: int = 30):
        ...
        if system.dimensions >= 3:
            ...
            ani = FuncAnimation(
                fig, update, frames=len(points_array), blit=True, interval=20
            )
