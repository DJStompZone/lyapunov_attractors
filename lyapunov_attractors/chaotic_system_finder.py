...
class ChaoticSystemFinder:
    ...
    def add_system(self, coefficients: List[float], lyapunov: float, points: List[List[float]]):
        """
    ...
    def search(self, num_attempts: int = None):
        ...
        print(f"Searching for chaotic systems with Lyapunov exponent >= {self.lyap_config.lyapunov_threshold}")
        print(f"Currently stored systems: {len(self.best_systems)}")

        for attempt in range(num_attempts):
            print(f"\rAttempt {attempt + 1}/{num_attempts}", end="")

            lyapunov, coefficients, points = self.simulator.simulate()

            if lyapunov > self.lyap_config.lyapunov_threshold:
                print(f"\nFound system with Lyapunov exponent: {lyapunov:.3f}")
                self.add_system(coefficients, lyapunov, points)

                print("Current best systems:")
                for idx, system in enumerate(self.best_systems, 1):
                    print(f"{idx}. Lyapunov: {system.lyapunov:.3f} (found: {system.timestamp})")
